# C1-5 — `confidential-fabric` RFC

> GPU/node attestation + confidential containers + policy-bound key release + encrypted model delivery.
...
**Goals:** GPU/node attestation + confidential containers + policy-bound key release + encrypted model delivery.
...
Implementation language(s): Rust + Go control. The component consumes the contract plane
...
**Dependency note:** where C1-5 depends on a Wave-2+ component not yet shipped (e.g. I1
...
- **Golden vectors:** `testvectors/C1-5/` — exercised by the cross-language conformance suite (A6).
