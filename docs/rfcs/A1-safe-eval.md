...
> YAML pipeline framework. Stages: benchmarks (HELM/LM-Eval), adversarial (garak/PyRIT), safety, bias, red_team (MDASH). Exports results into S4.
...
**Goals:** YAML pipeline framework. Stages: benchmarks (HELM/LM-Eval), adversarial (garak/PyRIT), safety, bias, red_team (MDASH). Exports results into S4.
...
(`proto/`, `specs/`, `testvectors/`) and depends on: I1, T1.
...
**Dependency note:** where A1 depends on a Wave-2+ component not yet shipped (e.g. I1
...
- **Golden vectors:** `testvectors/A1/` — exercised by the cross-language conformance suite (A6).
