# X4 — `crypto-audit-ai` RFC

> AI-assisted cryptanalysis. Modes: IMPLEMENTATION_AUDIT, ALGORITHM_STRESS_TEST, DEPENDENCY_SCAN. Integrates Anthropic CryptanalysisBench. Productizes Anthropic's research.

| Field | Value |
|---|---|
| **Canonical ID** | X4 |
| **Name** | crypto-audit-ai |
| **Wave** | 6 |
| **Languages** | Rust (eBPF) + Python drivers |
| **Warrantor origin** | F5 CryptoAuditAI |
...
Warrantor F5 CryptoAuditAI; AumSecure (none); Sentinel (none). The full strategic rationale
appears in the matrix entry and the originating source document (see
[`source-matrix/README.md`](../source-matrix/README.md)).

## Goals and Non-Goals

**Goals:** AI-assisted cryptanalysis. Modes: IMPLEMENTATION_AUDIT, ALGORITHM_STRESS_TEST, DEPENDENCY_SCAN. Integrates Anthropic CryptanalysisBench. Productizes Anthropic's research.
...
Implementation language(s): Rust (eBPF) + Python drivers. The component consumes the contract plane
(`proto/`, `specs/`, `testvectors/`) and depends on: T1, A1.
...
**Dependency note:** where X4 depends on a Wave-2+ component not yet shipped (e.g. I1
...
- **Golden vectors:** `testvectors/X4/` — exercised by the cross-language conformance suite (A6).
