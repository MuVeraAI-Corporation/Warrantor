# S1 — `safe-tensors-pp` RFC

> Drop-in extension of HF Safetensors with __provenance__ block (signer, Ed25519 sig, signed_at, evaluations, lineage). Backward-compatible. ATLAS sidecar .atlas is the same idea.

| Field | Value |
|---|---|
| **Canonical ID** | S1 |
| **Name** | safe-tensors-pp |
| **Wave** | 2 |
| **Languages** | Python + Rust core |
| **Warrantor origin** | C2.1 SafeTensors++ |
| **AumSecure origin** | (extends Safetensors) |
| **Sentinel origin** | atlas-format |
| **Dependencies** | T1 |
...
Warrantor C2.1 SafeTensors++; AumSecure (extends Safetensors); Sentinel atlas-format. The full strategic rationale
appears in the matrix entry and the originating source document (see
[`source-matrix/README.md`](../source-matrix/README.md)).

## Goals and Non-Goals

**Goals:** Drop-in extension of HF Safetensors with __provenance__ block (signer, Ed25519 sig, signed_at, evaluations, lineage). Backward-compatible. ATLAS sidecar .atlas is the same idea.
...
Implementation language(s): Python + Rust core. The component consumes the contract plane
(`proto/`, `specs/`, `testvectors/`) and depends on: T1.
...
**Dependency note:** where S1 depends on a Wave-2+ component not yet shipped (e.g. I1
...
- **Golden vectors:** `testvectors/S1/` — exercised by the cross-language conformance suite (A6).
