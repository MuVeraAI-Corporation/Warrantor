...
> Web UI for policy administration, evidence explorer, approvals, fleet management, compliance reports.
...
| **AumSecure origin** | Enterprise policy/evidence console |
| **Sentinel origin** | sentinelos-console |
| **Dependencies** | I1, E1 |
...
Warrantor (none); AumSecure Enterprise policy/evidence console; Sentinel sentinelos-console. The full strategic rationale
appears in the matrix entry and the originating source document (see
[`source-matrix/README.md`](../source-matrix/README.md)).

## Goals and Non-Goals

**Goals:** Web UI for policy administration, evidence explorer, approvals, fleet management, compliance reports.
...
Implementation language(s): TypeScript (Next.js). The component consumes the contract plane
(`proto/`, `specs/`, `testvectors/`) and depends on: I1, E1.
...
**Dependency note:** where X7 depends on a Wave-2+ component not yet shipped (e.g. I1
...
- **Golden vectors:** `testvectors/X7/` — exercised by the cross-language conformance suite (A6).
