...
Implementation language(s): Rust + eBPF (aya). The component consumes the contract plane
(`proto/`, `specs/`, `testvectors/`) and depends on: C1-2.
...
**Dependency note:** where R2 depends on a Wave-2+ component not yet shipped (e.g. I1
...
- **Golden vectors:** `testvectors/R2/` — exercised by the cross-language conformance suite (A6).
