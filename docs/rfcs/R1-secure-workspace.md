...
**Goals:** OpenShell-based isolated workspace. Signed policy, credential brokering, network allowlists, controlled inference, approval gates, full action evidence. R2 is its attestation arm.
...
**Dependency note:** where R1 depends on a Wave-2+ component not yet shipped (e.g. I1
...
- **Golden vectors:** `testvectors/R1/` — exercised by the cross-language conformance suite (A6).
