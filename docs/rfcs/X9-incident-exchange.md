# X9 — `incident-exchange` RFC

> Normalized agent incidents: goal hijack, memory poisoning, tool abuse, identity compromise, exfiltration, rogue delegation. OCSF extension + MITRE ATLAS mapping.

| Field | Value |
|---|---|
| **Canonical ID** | X9 |
| **Name** | incident-exchange |
| **Wave** | 6 |
| **Languages** | Spec + Python |
| **Warrantor origin** | (none) |
| **AumSecure origin** | agent-incident-exchange (V2 W2) + AIX protocol |
| **Sentinel origin** | AEGIS SOC + delta-notify |
| **Dependencies** | R3, E1 |
...
Warrantor (none); AumSecure agent-incident-exchange (V2 W2) + AIX protocol; Sentinel AEGIS SOC + delta-notify. The full strategic rationale
appears in the matrix entry and the originating source document (see
[`source-matrix/README.md`](../source-matrix/README.md)).

## Goals and Non-Goals

**Goals:** Normalized agent incidents: goal hijack, memory poisoning, tool abuse, identity compromise, exfiltration, rogue delegation. OCSF extension + MITRE ATLAS mapping.
...
Implementation language(s): Spec + Python. The component consumes the contract plane
(`proto/`, `specs/`, `testvectors/`) and depends on: R3, E1.
...
**Dependency note:** where X9 depends on a Wave-2+ component not yet shipped (e.g. I1
...
- **Golden vectors:** `testvectors/X9/` — exercised by the cross-language conformance suite (A6).
