...
> Model tamper detection. weight_distribution, backdoor_patterns, neuron_pruning, fine_tune_detection. AI equivalent of a virus scanner.
...
| **Wave** | 3 |
| **Languages** | Python |
| **Warrantor origin** | C4.4 TamperScan |
...
Warrantor C4.4 TamperScan; AumSecure (none); Sentinel (none). The full strategic rationale
appears in the matrix entry and the originating source document (see
[`source-matrix/README.md`](../source-matrix/README.md)).

## Goals and Non-Goals

**Goals:** Model tamper detection. weight_distribution, backdoor_patterns, neuron_pruning, fine_tune_detection. AI equivalent of a virus scanner.
...
(`proto/`, `specs/`, `testvectors/`) and depends on: S2.
...
**Dependency note:** where S7 depends on a Wave-2+ component not yet shipped (e.g. I1
...
- **Golden vectors:** `testvectors/S7/` — exercised by the cross-language conformance suite (A6).
