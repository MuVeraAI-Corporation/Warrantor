# C1-3 — `attesta-flow` RFC

> E2E attested inference pipeline. Terraform module (Azure DC, AWS Nitro, GCP CC VMs + NVIDIA). Python orchestrator in TEE. External verifier. Emits signed PipelineAttestation per batch.
...
| **Warrantor origin** | C1.2 AttestaFlow |
...
Warrantor C1.2 AttestaFlow; AumSecure (none); Sentinel (none). The full strategic rationale
appears in the matrix entry and the originating source document (see
[`source-matrix/README.md`](../source-matrix/README.md)).

## Goals and Non-Goals

**Goals:** E2E attested inference pipeline. Terraform module (Azure DC, AWS Nitro, GCP CC VMs + NVIDIA). Python orchestrator in TEE. External verifier. Emits signed PipelineAttestation per batch.
...
Implementation language(s): Python + Terraform. The component consumes the contract plane
(`proto/`, `specs/`, `testvectors/`) and depends on: C1-2, I1.
...
**Dependency note:** where C1-3 depends on a Wave-2+ component not yet shipped (e.g. I1
...
- **Golden vectors:** `testvectors/C1-3/` — exercised by the cross-language conformance suite (A6).
