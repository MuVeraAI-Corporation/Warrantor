...
> Unified adversarial testing. Wraps garak + PyRIT; built-in generators: PromptInjection, Jailbreak, EncodingAttack, MultiTurnManipulation, TrainingDataExtraction.
...
| **Name** | adversaria |
| **Wave** | 3 |
| **Languages** | Python + Rust orchestrator |
| **Warrantor origin** | C5.2 Adversaria |
| **AumSecure origin** | (none) |
| **Sentinel origin** | HYDRA heads |
| **Dependencies** | A1 |
...
Warrantor C5.2 Adversaria; AumSecure (none); Sentinel HYDRA heads. The full strategic rationale
appears in the matrix entry and the originating source document (see
[`source-matrix/README.md`](../source-matrix/README.md)).

## Goals and Non-Goals

**Goals:** Unified adversarial testing. Wraps garak + PyRIT; built-in generators: PromptInjection, Jailbreak, EncodingAttack, MultiTurnManipulation, TrainingDataExtraction.
...
Implementation language(s): Python + Rust orchestrator. The component consumes the contract plane
(`proto/`, `specs/`, `testvectors/`) and depends on: A1.
...
**Dependency note:** where A2 depends on a Wave-2+ component not yet shipped (e.g. I1
...
- **Golden vectors:** `testvectors/A2/` — exercised by the cross-language conformance suite (A6).
