...
> Cross-language conformance suite. CLI, GitHub Action, CI images. Verifies golden vectors across Rust/Python/TS/Go. Active in Wave-1 as tools/conformance/.
...
| **Name** | conformance |
| **Wave** | 2 |
| **Languages** | Rust ref + CLI + GitHub Action |
| **Warrantor origin** | (none) |
| **AumSecure origin** | agent-conformance (V3 repo #5) |
...
Warrantor (none); AumSecure agent-conformance (V3 repo #5); Sentinel (none). The full strategic rationale
appears in the matrix entry and the originating source document (see
[`source-matrix/README.md`](../source-matrix/README.md)).

## Goals and Non-Goals

**Goals:** Cross-language conformance suite. CLI, GitHub Action, CI images. Verifies golden vectors across Rust/Python/TS/Go. Active in Wave-1 as tools/conformance/.
...
Implementation language(s): Rust ref + CLI + GitHub Action. The component consumes the contract plane
...
**Dependency note:** where A6 depends on a Wave-2+ component not yet shipped (e.g. I1
...
- **Golden vectors:** `testvectors/A6/` — exercised by the cross-language conformance suite (A6).
