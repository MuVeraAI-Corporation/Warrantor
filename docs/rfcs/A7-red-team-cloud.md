# A7 — `red-team-cloud` RFC

> Continuous adversarial simulation as a service. Injection, tool poisoning, identity abuse, memory poisoning, rogue delegation, exfiltration, cascading failures.
...
| **AumSecure origin** | Agentic Red-Team & Evaluation Cloud (V2 #6) |
| **Sentinel origin** | AEGIS-Red + HYDRA |
| **Dependencies** | A5 |
...
Warrantor (none); AumSecure Agentic Red-Team & Evaluation Cloud (V2 #6); Sentinel AEGIS-Red + HYDRA. The full strategic rationale
appears in the matrix entry and the originating source document (see
[`source-matrix/README.md`](../source-matrix/README.md)).

## Goals and Non-Goals

**Goals:** Continuous adversarial simulation as a service. Injection, tool poisoning, identity abuse, memory poisoning, rogue delegation, exfiltration, cascading failures.
...
(`proto/`, `specs/`, `testvectors/`) and depends on: A5.
...
**Dependency note:** where A7 depends on a Wave-2+ component not yet shipped (e.g. I1
...
- **Golden vectors:** `testvectors/A7/` — exercised by the cross-language conformance suite (A6).
