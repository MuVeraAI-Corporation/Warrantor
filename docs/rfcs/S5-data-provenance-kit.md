# S5 — `data-provenance-kit` RFC

> Dataset lineage tracker. Wraps HF Datasets/S3/local; records every transformation as a node; exports signed JSON-LD.

| Field | Value |
|---|---|
| **Canonical ID** | S5 |
| **Name** | data-provenance-kit |
| **Wave** | 3 |
| **Languages** | Python |
| **Warrantor origin** | C4.2 DataProvenanceKit |
| **AumSecure origin** | (none) |
| **Sentinel origin** | (AGORA Catalog overlaps) |
| **Dependencies** | S4 |
...
Warrantor C4.2 DataProvenanceKit; AumSecure (none); Sentinel (AGORA Catalog overlaps). The full strategic rationale
appears in the matrix entry and the originating source document (see
[`source-matrix/README.md`](../source-matrix/README.md)).

## Goals and Non-Goals

**Goals:** Dataset lineage tracker. Wraps HF Datasets/S3/local; records every transformation as a node; exports signed JSON-LD.
...
(`proto/`, `specs/`, `testvectors/`) and depends on: S4.
...
**Dependency note:** where S5 depends on a Wave-2+ component not yet shipped (e.g. I1
...
- **Golden vectors:** `testvectors/S5/` — exercised by the cross-language conformance suite (A6).
