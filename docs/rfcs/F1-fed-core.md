...
> Attested federated training orchestration. Roles: Aggregator/Trainer/Verifier. PyTorch + NeMo. DP via F2. Most complex component in the portfolio.
...
| **Wave** | 5 |
| **Languages** | Python |
| **Warrantor origin** | C3.1 FedCore |
| **AumSecure origin** | (none) |
| **Sentinel origin** | AGORA federation |
| **Dependencies** | C1-2, I1, F2 |
...
Warrantor C3.1 FedCore; AumSecure (none); Sentinel AGORA federation. The full strategic rationale
appears in the matrix entry and the originating source document (see
[`source-matrix/README.md`](../source-matrix/README.md)).

## Goals and Non-Goals

**Goals:** Attested federated training orchestration. Roles: Aggregator/Trainer/Verifier. PyTorch + NeMo. DP via F2. Most complex component in the portfolio.
...
(`proto/`, `specs/`, `testvectors/`) and depends on: C1-2, I1, F2.
...
**Dependency note:** where F1 depends on a Wave-2+ component not yet shipped (e.g. I1
...
- **Golden vectors:** `testvectors/F1/` — exercised by the cross-language conformance suite (A6).
