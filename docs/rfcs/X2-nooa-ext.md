...
> Production extensions to NVIDIA NOOA. PolicyEnforcer (OPA/Rego), AuditStreamer (Kafka/Kinesis/webhook), IdentityBinder (SPIFFE), AttestationHook.
...
**Goals:** Production extensions to NVIDIA NOOA. PolicyEnforcer (OPA/Rego), AuditStreamer (Kafka/Kinesis/webhook), IdentityBinder (SPIFFE), AttestationHook.
...
**Dependency note:** where X2 depends on a Wave-2+ component not yet shipped (e.g. I1
...
- **Golden vectors:** `testvectors/X2/` — exercised by the cross-language conformance suite (A6).
