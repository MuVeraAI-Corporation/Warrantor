...
> Differential privacy toolkit. DPSGDOptimizer, PrivacyAccountant (moments accountant), DPDashboard. PyTorch + NeMo + Opacus-compatible.
...
| **Wave** | 5 |
| **Languages** | Python |
| **Warrantor origin** | C3.4 DPCrate |
| **AumSecure origin** | (none) |
| **Sentinel origin** | (AGORA DP) |
| **Dependencies** | none |
...
Warrantor C3.4 DPCrate; AumSecure (none); Sentinel (AGORA DP). The full strategic rationale
appears in the matrix entry and the originating source document (see
[`source-matrix/README.md`](../source-matrix/README.md)).

## Goals and Non-Goals

**Goals:** Differential privacy toolkit. DPSGDOptimizer, PrivacyAccountant (moments accountant), DPDashboard. PyTorch + NeMo + Opacus-compatible.
...
**Dependency note:** where F2 depends on a Wave-2+ component not yet shipped (e.g. I1
...
- **Golden vectors:** `testvectors/F2/` — exercised by the cross-language conformance suite (A6).
