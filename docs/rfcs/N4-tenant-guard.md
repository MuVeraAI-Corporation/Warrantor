...
> K8s operator. GPUQuota/TenantWorkload CRDs. NVIDIA MIG (HW) + MPS (SW) isolation. Per-tenant attestation. Go activation trigger #1.
...
| **Wave** | 4 |
| **Languages** | Go (gated) |
| **Warrantor origin** | C6.3 TenantGuard |
...
Warrantor C6.3 TenantGuard; AumSecure (none); Sentinel (none). The full strategic rationale
appears in the matrix entry and the originating source document (see
[`source-matrix/README.md`](../source-matrix/README.md)).

## Goals and Non-Goals

**Goals:** K8s operator. GPUQuota/TenantWorkload CRDs. NVIDIA MIG (HW) + MPS (SW) isolation. Per-tenant attestation. Go activation trigger #1.
...
(`proto/`, `specs/`, `testvectors/`) and depends on: I1, N1.
...
**Dependency note:** where N4 depends on a Wave-2+ component not yet shipped (e.g. I1
...
- **Golden vectors:** `testvectors/N4/` — exercised by the cross-language conformance suite (A6).
