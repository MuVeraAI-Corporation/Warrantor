...
> CI/CD compliance gates for AI. GitHub Action + GitLab CI template. .complygate.yml. Break-glass overrides with 2 mandatory approvers.
...
| **Warrantor origin** | C5.4 ComplyGate |
...
Warrantor C5.4 ComplyGate; AumSecure (none); Sentinel (none). The full strategic rationale
appears in the matrix entry and the originating source document (see
[`source-matrix/README.md`](../source-matrix/README.md)).

## Goals and Non-Goals

**Goals:** CI/CD compliance gates for AI. GitHub Action + GitLab CI template. .complygate.yml. Break-glass overrides with 2 mandatory approvers.
...
Implementation language(s): YAML + Python. The component consumes the contract plane
(`proto/`, `specs/`, `testvectors/`) and depends on: A1, T1.
...
**Dependency note:** where A4 depends on a Wave-2+ component not yet shipped (e.g. I1
...
- **Golden vectors:** `testvectors/A4/` — exercised by the cross-language conformance suite (A6).
