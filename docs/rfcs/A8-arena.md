...
> A/B Elo ranking leaderboard service. Dual leaderboard: defensive (most robust models) + offensive (most effective attacks).
...
| **Sentinel origin** | COLOSSEUM arena + leaderboards |
| **Dependencies** | A1 |
...
Warrantor (none); AumSecure (none); Sentinel COLOSSEUM arena + leaderboards. The full strategic rationale
appears in the matrix entry and the originating source document (see
[`source-matrix/README.md`](../source-matrix/README.md)).

## Goals and Non-Goals

**Goals:** A/B Elo ranking leaderboard service. Dual leaderboard: defensive (most robust models) + offensive (most effective attacks).
...
Implementation language(s): TypeScript + Go. The component consumes the contract plane
...
**Dependency note:** where A8 depends on a Wave-2+ component not yet shipped (e.g. I1
...
- **Golden vectors:** `testvectors/A8/` — exercised by the cross-language conformance suite (A6).
