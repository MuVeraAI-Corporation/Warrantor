# N3 — `inference-proxy` RFC

> LLM inference gateway. Middleware: auth (SPIFFE/API/OAuth), rate_limit, prompt_filter (injection/PII/policy), cache (semantic), audit, fallback.
...
| **Wave** | 4 |
| **Languages** | Rust + Go |
| **Warrantor origin** | C6.4 InferenceProxy |
...
Warrantor C6.4 InferenceProxy; AumSecure (none); Sentinel (none). The full strategic rationale
appears in the matrix entry and the originating source document (see
[`source-matrix/README.md`](../source-matrix/README.md)).

## Goals and Non-Goals

**Goals:** LLM inference gateway. Middleware: auth (SPIFFE/API/OAuth), rate_limit, prompt_filter (injection/PII/policy), cache (semantic), audit, fallback.
...
Implementation language(s): Rust + Go. The component consumes the contract plane
(`proto/`, `specs/`, `testvectors/`) and depends on: N1, I1.
...
**Dependency note:** where N3 depends on a Wave-2+ component not yet shipped (e.g. I1
...
- **Golden vectors:** `testvectors/N3/` — exercised by the cross-language conformance suite (A6).
