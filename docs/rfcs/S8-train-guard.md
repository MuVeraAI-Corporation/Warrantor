...
> Training-time integrity monitor. Hooks PyTorch training loop; checks gradient distributions, loss curves, dependency integrity, weight init; emits signed training attestation.
...
| **Wave** | 3 |
| **Languages** | Python |
| **Warrantor origin** | C4.3 TrainGuard |
...
Warrantor C4.3 TrainGuard; AumSecure (none); Sentinel (none). The full strategic rationale
appears in the matrix entry and the originating source document (see
[`source-matrix/README.md`](../source-matrix/README.md)).

## Goals and Non-Goals

**Goals:** Training-time integrity monitor. Hooks PyTorch training loop; checks gradient distributions, loss curves, dependency integrity, weight init; emits signed training attestation.
...
**Dependency note:** where S8 depends on a Wave-2+ component not yet shipped (e.g. I1
...
- **Golden vectors:** `testvectors/S8/` — exercised by the cross-language conformance suite (A6).
