# C1-1 — `nvtrust-bridge` RFC

> NVTrust FFI bindings + nvtrust-verify CLI. Offline/mock mode for CI; reference verifiers for H100/H200.
...
| **Name** | nvtrust-bridge |
| **Wave** | 1 |
| **Languages** | Rust core + Python + Go bindings |
| **Warrantor origin** | C1.4 NVTrustBridge |
...
Warrantor C1.4 NVTrustBridge; AumSecure (none); Sentinel (none). The full strategic rationale
appears in the matrix entry and the originating source document (see
[`source-matrix/README.md`](../source-matrix/README.md)).

## Goals and Non-Goals

**Goals:** NVTrust FFI bindings + nvtrust-verify CLI. Offline/mock mode for CI; reference verifiers for H100/H200.
...
Implementation language(s): Rust core + Python + Go bindings. The component consumes the contract plane
...
**Dependency note:** where C1-1 depends on a Wave-2+ component not yet shipped (e.g. I1
...
- **Golden vectors:** `testvectors/C1-1/` — exercised by the cross-language conformance suite (A6).
