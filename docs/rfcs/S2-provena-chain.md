# S2 — `provena-chain` RFC

> Tamper-evident provenance ledger; Merkle root published to Sigstore Rekor or blockchain. Required for EU AI Act lineage (Art. 55).
...
**Goals:** Tamper-evident provenance ledger; Merkle root published to Sigstore Rekor or blockchain. Required for EU AI Act lineage (Art. 55).
...
Implementation language(s): Rust (Merkle) + Python API. The component consumes the contract plane
(`proto/`, `specs/`, `testvectors/`) and depends on: T1, S1.
...
**Dependency note:** where S2 depends on a Wave-2+ component not yet shipped (e.g. I1
...
- **Golden vectors:** `testvectors/S2/` — exercised by the cross-language conformance suite (A6).
