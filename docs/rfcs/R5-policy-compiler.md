# R5 — `policy-compiler` RFC

> Compiles NL intent + enterprise rules + regulatory controls into OpenShell policy + OPA/Cedar rules + test cases.
...
| **Wave** | 6 |
| **Languages** | Rust core + TS UI |
| **Warrantor origin** | (none) |
| **AumSecure origin** | Agent Policy Compiler (V2 #8) |
...
Warrantor (none); AumSecure Agent Policy Compiler (V2 #8); Sentinel (none). The full strategic rationale
appears in the matrix entry and the originating source document (see
[`source-matrix/README.md`](../source-matrix/README.md)).

## Goals and Non-Goals

**Goals:** Compiles NL intent + enterprise rules + regulatory controls into OpenShell policy + OPA/Cedar rules + test cases.
...
Implementation language(s): Rust core + TS UI. The component consumes the contract plane
...
**Dependency note:** where R5 depends on a Wave-2+ component not yet shipped (e.g. I1
...
- **Golden vectors:** `testvectors/R5/` — exercised by the cross-language conformance suite (A6).
