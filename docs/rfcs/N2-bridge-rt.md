...
> Unified backend abstraction. Auto-selects TRT-LLM > vLLM > Ollama. Handles TRT-LLM v0.16 sampler_type at runtime. Auto-converts SafeTensors to TRT engine.
...
| **Wave** | 4 |
| **Languages** | Python + Go |
| **Warrantor origin** | C6.2 BridgeRT |
...
Warrantor C6.2 BridgeRT; AumSecure (none); Sentinel (none). The full strategic rationale
appears in the matrix entry and the originating source document (see
[`source-matrix/README.md`](../source-matrix/README.md)).

## Goals and Non-Goals

**Goals:** Unified backend abstraction. Auto-selects TRT-LLM > vLLM > Ollama. Handles TRT-LLM v0.16 sampler_type at runtime. Auto-converts SafeTensors to TRT engine.
...
Implementation language(s): Python + Go. The component consumes the contract plane
(`proto/`, `specs/`, `testvectors/`) and depends on: N1.
...
**Dependency note:** where N2 depends on a Wave-2+ component not yet shipped (e.g. I1
...
- **Golden vectors:** `testvectors/N2/` — exercised by the cross-language conformance suite (A6).
