...
> eBPF exfiltration prevention. PatternMatcher (AWS/GitHub/OpenAI keys, SSNs, CCs), EntropyDetector (4.5 min_entropy, 32 min_length), VolumeMonitor (1MB/transfer, 10MB/hr). Integrates Falco + Tetragon.
...
| **Name** | exfil-guard |
| **Wave** | 6 |
| **Languages** | Rust eBPF (Falco + Tetragon) |
| **Warrantor origin** | F7 ExfilGuard |
...
Warrantor F7 ExfilGuard; AumSecure (none); Sentinel (none). The full strategic rationale
appears in the matrix entry and the originating source document (see
[`source-matrix/README.md`](../source-matrix/README.md)).

## Goals and Non-Goals

**Goals:** eBPF exfiltration prevention. PatternMatcher (AWS/GitHub/OpenAI keys, SSNs, CCs), EntropyDetector (4.5 min_entropy, 32 min_length), VolumeMonitor (1MB/transfer, 10MB/hr). Integrates Falco + Tetragon.
...
Implementation language(s): Rust eBPF (Falco + Tetragon). The component consumes the contract plane
(`proto/`, `specs/`, `testvectors/`) and depends on: R3, A1.
...
**Dependency note:** where S6 depends on a Wave-2+ component not yet shipped (e.g. I1
...
- **Golden vectors:** `testvectors/S6/` — exercised by the cross-language conformance suite (A6).
