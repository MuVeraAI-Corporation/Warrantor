# T2 — `authority-spec` RFC

> Normative spec for the Agent Authority Envelope (P1 AAE). Issuer, subject, purpose, resources, tools, data classes, side-effect class, budget, geography, delegation depth, approvals, expiry, revocation.
...
| **Name** | authority-spec |
| **Wave** | 2 |
| **Languages** | Spec (JSON-Schema/CBOR/CDDL) + Rust ref |
| **Warrantor origin** | (none) |
| **AumSecure origin** | agent-authority-spec (V3 repo #1) |
...
Warrantor (none); AumSecure agent-authority-spec (V3 repo #1); Sentinel (none). The full strategic rationale
appears in the matrix entry and the originating source document (see
[`source-matrix/README.md`](../source-matrix/README.md)).

## Goals and Non-Goals

**Goals:** Normative spec for the Agent Authority Envelope (P1 AAE). Issuer, subject, purpose, resources, tools, data classes, side-effect class, budget, geography, delegation depth, approvals, expiry, revocation.
...
Implementation language(s): Spec (JSON-Schema/CBOR/CDDL) + Rust ref. The component consumes the contract plane
(`proto/`, `specs/`, `testvectors/`) and depends on: none.
...
**Dependency note:** where T2 depends on a Wave-2+ component not yet shipped (e.g. I1
...
- **Golden vectors:** `testvectors/T2/` — exercised by the cross-language conformance suite (A6).
