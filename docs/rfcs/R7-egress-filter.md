# R7 — `egress-filter` RFC

> eBPF egress enforcement. Policy/decision half of S6 exfil-guard. Domain blocklist + canary IPs (huggingface.co, pypi.org, 1.1.1.1).
...
Warrantor (none); AumSecure (none); Sentinel (none). The full strategic rationale
appears in the matrix entry and the originating source document (see
[`source-matrix/README.md`](../source-matrix/README.md)).

## Goals and Non-Goals

**Goals:** eBPF egress enforcement. Policy/decision half of S6 exfil-guard. Domain blocklist + canary IPs (huggingface.co, pypi.org, 1.1.1.1).
...
Implementation language(s): Rust eBPF. The component consumes the contract plane
(`proto/`, `specs/`, `testvectors/`) and depends on: R3, T1.
...
**Dependency note:** where R7 depends on a Wave-2+ component not yet shipped (e.g. I1
...
- **Golden vectors:** `testvectors/R7/` — exercised by the cross-language conformance suite (A6).
