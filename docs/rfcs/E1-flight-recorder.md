# E1 — `flight-recorder` RFC

> Emits signed Agent Action Receipts (P2 AAR) before commit. Framework-neutral. Export OCSF + OpenTelemetry.
...
| **Wave** | 2 |
| **Languages** | Rust core + TS viewer |
| **Warrantor origin** | (none) |
| **AumSecure origin** | Verifiable Agent Flight Recorder (V2 #3) + agent-evidence receipt half |
...
Warrantor (none); AumSecure Verifiable Agent Flight Recorder (V2 #3) + agent-evidence receipt half; Sentinel (none). The full strategic rationale
appears in the matrix entry and the originating source document (see
[`source-matrix/README.md`](../source-matrix/README.md)).

## Goals and Non-Goals

**Goals:** Emits signed Agent Action Receipts (P2 AAR) before commit. Framework-neutral. Export OCSF + OpenTelemetry.
...
Implementation language(s): Rust core + TS viewer. The component consumes the contract plane
...
**Dependency note:** where E1 depends on a Wave-2+ component not yet shipped (e.g. I1
...
- **Golden vectors:** `testvectors/E1/` — exercised by the cross-language conformance suite (A6).
