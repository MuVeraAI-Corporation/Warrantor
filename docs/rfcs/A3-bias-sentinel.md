# A3 — `bias-sentinel` RFC

> Combined bias + copyright auditing. Bias (BOLD, HONEST, CrowS-Pairs, WinoBias) + copyright (n-gram overlap, fuzzy). EU AI Act copyright compliance.
...
| **Name** | bias-sentinel |
| **Wave** | 6 |
| **Languages** | Python + TS dashboard |
| **Warrantor origin** | C5.3 BiasSentinel |
...
Warrantor C5.3 BiasSentinel; AumSecure (none); Sentinel (none). The full strategic rationale
appears in the matrix entry and the originating source document (see
[`source-matrix/README.md`](../source-matrix/README.md)).

## Goals and Non-Goals

**Goals:** Combined bias + copyright auditing. Bias (BOLD, HONEST, CrowS-Pairs, WinoBias) + copyright (n-gram overlap, fuzzy). EU AI Act copyright compliance.
...
Implementation language(s): Python + TS dashboard. The component consumes the contract plane
...
**Dependency note:** where A3 depends on a Wave-2+ component not yet shipped (e.g. I1
...
- **Golden vectors:** `testvectors/A3/` — exercised by the cross-language conformance suite (A6).
