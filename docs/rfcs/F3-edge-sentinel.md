# F3 — `edge-sentinel` RFC

> Edge inference attestation agent. Jetson/EGX. systemd service. Kills inference on tamper; alerts F4.
...
**Goals:** Edge inference attestation agent. Jetson/EGX. systemd service. Kills inference on tamper; alerts F4.
...
Implementation language(s): Go (<5MB). The component consumes the contract plane
(`proto/`, `specs/`, `testvectors/`) and depends on: T1, F4.
...
**Dependency note:** where F3 depends on a Wave-2+ component not yet shipped (e.g. I1
...
- **Golden vectors:** `testvectors/F3/` — exercised by the cross-language conformance suite (A6).
