...
> TEE-backed model serving. Go sidecar to Triton/vLLM. TLS terminates in TEE; forwards via UDS; wraps responses in AttestationEnvelope. <2ms overhead.
...
| **Wave** | 5 |
| **Languages** | Go (gated) |
| **Warrantor origin** | C1.3 TeeServe |
...
Warrantor C1.3 TeeServe; AumSecure (none); Sentinel (none). The full strategic rationale
appears in the matrix entry and the originating source document (see
[`source-matrix/README.md`](../source-matrix/README.md)).

## Goals and Non-Goals

**Goals:** TEE-backed model serving. Go sidecar to Triton/vLLM. TLS terminates in TEE; forwards via UDS; wraps responses in AttestationEnvelope. <2ms overhead.
...
**Dependency note:** where C1-4 depends on a Wave-2+ component not yet shipped (e.g. I1
...
- **Golden vectors:** `testvectors/C1-4/` — exercised by the cross-language conformance suite (A6).
