...
Implementation language(s): Rust. The component consumes the contract plane
(`proto/`, `specs/`, `testvectors/`) and depends on: R3 (mock).
...
**Dependency note:** where R4 depends on a Wave-2+ component not yet shipped (e.g. I1
...
- **Golden vectors:** `testvectors/R4/` — exercised by the cross-language conformance suite (A6).
