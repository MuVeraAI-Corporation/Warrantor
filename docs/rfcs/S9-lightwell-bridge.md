# S9 — `lightwell-bridge` RFC

> AI-artifact patch distribution extending IBM/Red Hat Lightwell. Model weight updates, guardrail updates, config changes, runtime updates.

| Field | Value |
|---|---|
| **Canonical ID** | S9 |
| **Name** | lightwell-bridge |
| **Wave** | 6 |
| **Languages** | Go + Rust |
| **Warrantor origin** | (none) |
| **AumSecure origin** | (none) |
| **Sentinel origin** | delta-patch (extends Lightwell) |
| **Dependencies** | S4, X9 |
...
Warrantor (none); AumSecure (none); Sentinel delta-patch (extends Lightwell). The full strategic rationale
appears in the matrix entry and the originating source document (see
[`source-matrix/README.md`](../source-matrix/README.md)).

## Goals and Non-Goals

**Goals:** AI-artifact patch distribution extending IBM/Red Hat Lightwell. Model weight updates, guardrail updates, config changes, runtime updates.
...
Implementation language(s): Go + Rust. The component consumes the contract plane
(`proto/`, `specs/`, `testvectors/`) and depends on: S4, X9.
...
**Dependency note:** where S9 depends on a Wave-2+ component not yet shipped (e.g. I1
...
- **Golden vectors:** `testvectors/S9/` — exercised by the cross-language conformance suite (A6).
