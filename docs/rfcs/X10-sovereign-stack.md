# X10 — `sovereign-stack` RFC

> Air-gapped single-node sovereign bundle. RTX/Linux + import/export bundles. Three modes: Safe Local / Safe Team / Safe Production.
...
| **Wave** | 7 |
| **Languages** | Bash + Ansible + Helm |
| **Warrantor origin** | (none) |
| **AumSecure origin** | (none) |
| **Sentinel origin** | SENTINEL-OS Core + sovereign-agent-stack |
| **Dependencies** | all |
...
Warrantor (none); AumSecure (none); Sentinel SENTINEL-OS Core + sovereign-agent-stack. The full strategic rationale
appears in the matrix entry and the originating source document (see
[`source-matrix/README.md`](../source-matrix/README.md)).

## Goals and Non-Goals

**Goals:** Air-gapped single-node sovereign bundle. RTX/Linux + import/export bundles. Three modes: Safe Local / Safe Team / Safe Production.
...
Implementation language(s): Bash + Ansible + Helm. The component consumes the contract plane
(`proto/`, `specs/`, `testvectors/`) and depends on: all.
...
**Dependency note:** where X10 depends on a Wave-2+ component not yet shipped (e.g. I1
...
- **Golden vectors:** `testvectors/X10/` — exercised by the cross-language conformance suite (A6).
