# N1 — `open-serve-kit` RFC

> OpenAI-compatible proxy; backend-agnostic (vLLM, TensorRT-LLM, Triton, Ollama). Optional attestation envelope per response. Go cleared (real serving service).
...
| **AumSecure origin** | C6.1 OpenServeKit |
| **Sentinel origin** | (none) |
| **Dependencies** | I1, C1-2 |
...
Warrantor (none); AumSecure C6.1 OpenServeKit; Sentinel (none). The full strategic rationale
appears in the matrix entry and the originating source document (see
[`source-matrix/README.md`](../source-matrix/README.md)).

## Goals and Non-Goals

**Goals:** OpenAI-compatible proxy; backend-agnostic (vLLM, TensorRT-LLM, Triton, Ollama). Optional attestation envelope per response. Go cleared (real serving service).
...
Implementation language(s): Go (gated). The component consumes the contract plane
(`proto/`, `specs/`, `testvectors/`) and depends on: I1, C1-2.
...
**Dependency note:** where N1 depends on a Wave-2+ component not yet shipped (e.g. I1
...
- **Golden vectors:** `testvectors/N1/` — exercised by the cross-language conformance suite (A6).
