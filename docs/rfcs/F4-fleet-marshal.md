# F4 — `fleet-marshal` RFC

> K8s operator. ModelFleet CRD. Canary/blue-green/all-at-once OTA. Auto-rollback at failure threshold. Go activation trigger #2.
...
| **Wave** | 5 |
| **Languages** | Go (gated) |
| **Warrantor origin** | C3.3 FleetMarshal |
...
Warrantor C3.3 FleetMarshal; AumSecure (none); Sentinel (none). The full strategic rationale
appears in the matrix entry and the originating source document (see
[`source-matrix/README.md`](../source-matrix/README.md)).

## Goals and Non-Goals

**Goals:** K8s operator. ModelFleet CRD. Canary/blue-green/all-at-once OTA. Auto-rollback at failure threshold. Go activation trigger #2.
...
**Dependency note:** where F4 depends on a Wave-2+ component not yet shipped (e.g. I1
...
- **Golden vectors:** `testvectors/F4/` — exercised by the cross-language conformance suite (A6).
