...
- **M-C deferred here, and already answered elsewhere.** It requires verdict-token probabilities and
  ...
  eliminate (§3.4). LongGuard supplies the attention→logit→behavior chain we deferred.
...
identical to every other measurement in this program. **Zero unparsed outputs across 2,000
evaluations.**
...
M-C requires verdict-token *probabilities*, not argmax verdicts. The serving stack used throughout
this program does not usefully expose logprobs, so M-C requires `transformers` or vLLM.
...
*understand* the payload. It is failing to *attend* to it.
