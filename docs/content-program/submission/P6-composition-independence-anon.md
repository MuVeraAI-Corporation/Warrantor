...
verdict sets come from our own earlier panel experiment [Anon-A §5.7], pre-registered and hashed before
its data existed.
...
explicitly weaker than the others in this series and says so in its own §0**: the panel data already
...
would be the one-directional error companion work documents [Anon-A §5.1], and the pre-registration
