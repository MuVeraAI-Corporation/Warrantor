...
#: The publication set. P2 is absent on purpose -- never published, novelty assessed as weakened,
#: and not carried through the draft-apparatus edits -- so the gate below would fail it for
#: reasons nobody has decided to fix. See the same note in to_latex.py.
PAPERS = [
...
def build(tag, name):
    ...
    md = T.strip_draft_apparatus(md)
    # Submission-mode notes are artifacts of the anonymous build, not content. None may survive
    # into a named preprint that carries the author's name three lines above it. Three forms
    # occur in the sources and each is removed explicitly, because a note that reads "no author
    # is identified" printed under the author's name and ORCID is the kind of contradiction a
    # moderator stops at.
    md = re.sub(r"^\*Submitted to [^\n]*\*\s*$", "", md, flags=re.M)
    md = re.sub(r"^> Written in third person for double-blind\.(?:\n>[^\n]*)*\n?", "",
                md, flags=re.M)
    md = re.sub(r"^> \*\*Double-blind note[^\n]*(?:\n>[^\n]*)*\n?", "", md, flags=re.M)
    ...
    keywords = (T.ACM_CCS.get(tag) or {}).get("keywords", "")
    doc = [f"% PREPRINT (NAMED) -- {tag}. Generated by build_preprint.py; do not edit by hand.",
           ...
           # Document properties. This is the named build, so the author goes in too.
           T.pdf_metadata_block(t_title, keywords, author=t_author),
    ...
    apparatus = T.residual_draft_apparatus(out, named=True)
    ...
    if apparatus:
        # Written so it can be inspected; FAILED so it cannot ship. The previous published set
        # went out with exactly these strings in it and was rejected.
        print(f"      DRAFT APPARATUS: {len(apparatus)} hit(s)")
        for why, ctx in apparatus[:8]:
            print(f"        [{why}] ...{ctx}...")
        return False
    return not bad
...
def main():
    ...
    for tag, name in PAPERS:
        ok &= build(tag, name)
    print("\nAll built." if ok else "\nFAILED: at least one paper did not pass the build gates above.")
