...
\usepackage{lmodern,microtype,booktabs,longtable,array,calc,amsmath,amssymb,url}
...
\title{\textbf{Masking a Field's Loss Does Not Isolate That Field:\\
Two Rejected LoRA Runs on a Structured-Output Guard Model}}
\author{Anonymous Author(s)\\Submission to AIWILD @ NeurIPS 2026\\Under double-blind review}
\date{}
\begin{document}
\maketitle
\begin{abstract}
...
\section{Why report two runs that were rejected}
...
\section{Run 1 --- supervising a three-valued field with binary targets}

\subsection{The gate rejected it, and was right twice}
...
\subsection{Against the same-size baseline, the fine-tune made the model worse}
...
\subsection{The second-order failure: a documented lever became a no-op}
...
\subsection{Targeting the weak category did not fix the weak category}
...
\subsection{The repair that was not available}
...
\section{Run 2 --- masking the field, and what the field did}
...
\subsection{The distributional signature}
...
\textbf{The raw per-item outputs no longer exist.} The results directory was excluded from version
