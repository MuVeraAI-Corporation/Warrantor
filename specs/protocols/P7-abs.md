# P7 — Autonomy Budget Specification

> Machine-enforceable budgets: steps, wall-clock time, tokens, money, external calls, data volume, privilege, irreversible actions, expected risk.

| Field | Value |
|---|---|
| **Protocol ID** | P7 |
| **Name** | abs (Autonomy Budget Specification) |
| **Spec-only canonical** | Yes — see [`../../docs/00-reconciliation-matrix.md`](../../docs/00-reconciliation-matrix.md) §9 |
| **Consumed by** | I1, R3 |
| **Schema location** | `specs/protocols/P7-abs.schema.json + .cddl` |
...
The normative schema lives at `specs/protocols/P7-abs.schema.json + .cddl`. Mandatory fields:

```
envelope: protocol, version, message_id, issuer, issued_at, expires_at, nonce, critical_extensions, extensions
payload:  budget, currency, privilege, expected_risk_micros, approval_required, replenishment
...
Each protocol ships adversarial test vectors in `testvectors/protocols/P7/`:
