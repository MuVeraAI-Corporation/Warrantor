# P4 — Agent Memory Integrity Ledger

> Prevents silent memory poisoning. Signed records with ownership, confidence, contradiction links, provenance, quarantine, supersession, retention, consent revocation.

| Field | Value |
|---|---|
| **Protocol ID** | P4 |
| **Name** | amil (Agent Memory Integrity Ledger) |
| **Spec-only canonical** | Yes — see [`../../docs/00-reconciliation-matrix.md`](../../docs/00-reconciliation-matrix.md) §9 |
| **Consumed by** | future context/memory components |
| **Schema location** | `specs/protocols/P4-amil.schema.json + .cddl` |
...
The normative schema lives at `specs/protocols/P4-amil.schema.json + .cddl`. Mandatory fields:

```
envelope: protocol, version, message_id, issuer, issued_at, expires_at, nonce, critical_extensions, extensions
payload:  sequence, previous_digest, owner, content_digest, confidence_micros, contradiction_links, provenance_digest, quarantine_state, supersedes, retention_until, consent_revoked
...
Each protocol ships adversarial test vectors in `testvectors/protocols/P4/`:
