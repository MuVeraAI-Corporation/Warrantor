# P5 — Secure Skill Package

> Distribute skills safely. Bundles instructions, code, tools, permissions, publisher identity, OMS signature, AI-SBOM, eval set, benchmark, limitations, revocation.

| Field | Value |
|---|---|
| **Protocol ID** | P5 |
| **Name** | ssp (Secure Skill Package) |
| **Spec-only canonical** | Yes — see [`../../docs/00-reconciliation-matrix.md`](../../docs/00-reconciliation-matrix.md) §9 |
| **Consumed by** | S4, X8 |
| **Schema location** | `specs/protocols/P5-ssp.schema.json + .cddl` |
...
The normative schema lives at `specs/protocols/P5-ssp.schema.json + .cddl`. Mandatory fields:

```
envelope: protocol, version, message_id, issuer, issued_at, expires_at, nonce, critical_extensions, extensions
payload:  name, package, instructions_digest, code, runtime, tools, permissions, publisher_identity, ai_sbom, evaluation_bundle, limitations, revocation_handle
...
Each protocol ships adversarial test vectors in `testvectors/protocols/P5/`:
