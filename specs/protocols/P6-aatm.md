...
| **Schema location** | `specs/protocols/P6-aatm.schema.json + .cddl` |
...
The normative schema lives at `specs/protocols/P6-aatm.schema.json + .cddl`. Mandatory fields:

```
envelope: protocol, version, message_id, issuer, issued_at, expires_at, nonce, critical_extensions, extensions
payload:  artifacts, roles, root_digest, build_provenance, deployment_attestations
...
Each protocol ships adversarial test vectors in `testvectors/protocols/P6/`:
