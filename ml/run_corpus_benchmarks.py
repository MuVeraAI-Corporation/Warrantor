...
BENCHMARKS = (
    ("wildguard", "benchmark_wildguard.py", "WildGuardTest -- 47% adversarial, the recall figure"),
    ("expguard", "benchmark_expguard.py", "ExpGuardTest -- the false-positive-rate figure"),
)
...
def preflight(endpoint: str) -> list[str]:
    ...
    for _, script, _ in BENCHMARKS:
        if not (HERE / script).exists():
            problems.append(f"{HERE / script} is missing")
...
def main() -> int:
    ...
    parser.add_argument(
        "--out-dir",
        type=Path,
        default=HERE.parent / "eval_results",
    ...
    for name, script, what in BENCHMARKS:
        ...
        if result.returncode != 0:
            ...
            print(f"    {name} exited {result.returncode}; continuing to the next.", file=sys.stderr)
        else:
            print(f"    wrote {out}")
