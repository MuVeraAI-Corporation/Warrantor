#!/usr/bin/env python3
"""CLI entry point for ``warrantor_ml.model_card``.

The implementation lives at ``python/warrantor_ml/src/warrantor_ml/model_card.py``, inside the
...
Running ``python ml/model_card.py`` puts this directory on ``sys.path`` automatically, which is
what makes the ``_bootstrap`` import below resolve in a bare checkout.

Run: ``python ml/model_card.py --help``
"""
...
from warrantor_ml.model_card import main  # noqa: E402
