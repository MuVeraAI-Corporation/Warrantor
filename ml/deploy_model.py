#!/usr/bin/env python3
"""CLI entry point for ``warrantor_ml.deploy_model``.

The implementation lives at ``python/warrantor_ml/src/warrantor_ml/deploy_model.py``, inside the
...
Running ``python ml/deploy_model.py`` puts this directory on ``sys.path`` automatically, which is
what makes the ``_bootstrap`` import below resolve in a bare checkout.

Run: ``python ml/deploy_model.py --help``
"""
...
from warrantor_ml.deploy_model import main  # noqa: E402
