#!/usr/bin/env python3
"""CLI entry point for ``warrantor_ml.fine_tune``.

The implementation lives at ``python/warrantor_ml/src/warrantor_ml/fine_tune.py``, inside the
...
Running ``python ml/fine_tune.py`` puts this directory on ``sys.path`` automatically, which is
what makes the ``_bootstrap`` import below resolve in a bare checkout.

Run: ``python ml/fine_tune.py --help``
"""
...
from warrantor_ml.fine_tune import main  # noqa: E402
