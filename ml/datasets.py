#!/usr/bin/env python3
"""CLI entry point for ``warrantor_ml.datasets``.

The implementation lives at ``python/warrantor_ml/src/warrantor_ml/datasets.py``, inside the
...
Running ``python ml/datasets.py`` puts this directory on ``sys.path`` automatically, which is
what makes the ``_bootstrap`` import below resolve in a bare checkout.

Run: ``python ml/datasets.py --help``
"""
...
from warrantor_ml.datasets import main  # noqa: E402
