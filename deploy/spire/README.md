# SPIRE mTLS deployment for the AumOS trust domain (`warrantor.dev`)
...
The trust domain is `warrantor.dev`, matching `global.trustDomain` in
...
| `trust-domain-config.yaml` | Trust-domain policy: workload registration entries (`spiffe://warrantor.dev/<service>`), federation to `eu.warrantor.dev`, and an mTLS-scoped NetworkPolicy. |

## Architecture

```
                       ┌────────────────────────────────────────────┐
                       │              Trust domain: warrantor.dev         │
                       └────────────────────────────────────────────┘
                                          │
...
openssl req -x509 -newkey rsa:4096 -sha256 -days 3650 -nodes \
  ...
  -subj "/O=AumOS — Open Secure AI Alliance/CN=warrantor.dev SPIRE CA"
...
peers by their SPIFFE ID (`spiffe://warrantor.dev/inference-proxy`) over mTLS.
...
To federate with a second region (e.g. `eu.warrantor.dev`):

1. Stand up a SPIRE stack in the EU cluster with `trust_domain = "eu.warrantor.dev"`.
2. Apply the `SPIFFEBundleEndpoint` in `trust-domain-config.yaml` — it pulls the
   peer bundle from `https://spire-server.eu.warrantor.dev:8443` every 5 minutes.
3. Add `eu.warrantor.dev` to the `federatesWith` list of any entry that needs to
