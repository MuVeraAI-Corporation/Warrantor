...
# guard-lora-t1b-attention — a guard adapter that lost an output class

**LoRA adapter on `Qwen/Qwen3Guard-Gen-0.6B`, target modules: attention only.**

Attention projections only. Highest recall in the programme and the worst model in it: the gain is substantially bought with false positives, and the severity class is gone.
...
| **this adapter** | 0.8439 | 0.1649 | 1047 | 1227 | **0** |

*ExpGuardTest, n = 2,275, `num_ctx` 8192, seed 0, greedy. Paired exact-binomial McNemar against the base: 2 items the base caught and this missed, 164 the reverse, p < 0.0001.*
...
```bibtex
@misc{jha2026guardlorat1battention,
  author = {Vikram Jha},
