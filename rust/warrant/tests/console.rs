...
//! reader is told "no warrants yet", "no warrants in this state" or "the server did not answer" —
//! cannot be exercised from here. There is no JavaScript runner, and RFC W1 §Dependencies forbids
//! adding one ("No JavaScript build step, framework, bundler or package manager"). It is covered
//! by the manual run recorded in W1 §Testing, and this note is the honest statement of the gap
//! rather than a coverage number that would imply otherwise.
...
fn has_inline_event_handler(document: &str) -> bool {
    document.match_indices(" on").any(|(at, _)| {
        let rest = &document[at + 3..];
...
fn the_console_explains_what_a_warrant_is_before_there_is_one_to_look_at() {
    ...
    // The second sentence is the one that makes the first safe to believe: it says where the work
    // goes and what stops it escaping, in the CLI's own vocabulary rather than a second one.
    assert!(flat.contains("isolated copy of your repository"));
    assert!(flat.contains("until a person settles the warrant"));
    assert!(flat.contains("staged rather than performed"));
}
...
fn the_first_run_panel_carries_the_grant_command_verbatim() {
    ...
    assert!(
        flat.contains(r#"warrantor grant --goal "..." --tools git --write src"#),
        "the grant line must appear exactly as it is typed"
    );
    assert!(
...
fn an_empty_store_and_an_empty_filter_are_different_sentences() {
    ...
    assert!(
        flat.contains("The server did not answer."),
