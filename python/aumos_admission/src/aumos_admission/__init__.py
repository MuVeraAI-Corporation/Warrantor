"""AumOS Kubernetes Admission Controller.

A validating admission webhook that refuses to admit a Pod into a cluster
unless it carries a valid **AumOS Attestation Envelope (AAE)** annotation.
...
class AdmissionWebhook:
    ...
    required_annotations: set[str] = field(
        default_factory=lambda: {DEFAULT_AAE_ANNOTATION}
    )
    ...
    def validate_pod(self, pod_manifest: dict[str, Any]) -> ValidationResult:
        ...
        if missing:
            return ValidationResult(
                allowed=False,
                reason=(
                    "missing required AumOS attestation annotation(s): "
                    + ", ".join(missing)
                ),
                annotations_required=sorted(self.required_annotations),
                pod_name=name,
        ...
        if self.digest_annotation:
            ...
            if digest and not _DIGEST_RE.match(str(digest)):
                return ValidationResult(
                    allowed=False,
                    reason=(
                        f"annotation {self.digest_annotation!r} must match "
                        "'sha256:<hex>'"
                    ),
                    annotations_required=sorted(self.required_annotations),
                    pod_name=name,
