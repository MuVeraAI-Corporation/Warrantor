"""Tests for the DP1 data plane verify path."""

from __future__ import annotations

import pytest

import warrantor_data_plane as wdp


def test_canonical_body_sorted():
    assert wdp.canonical_body({"b": 2, "a": 1}) == '{"a":1,"b":2}'


def test_verify_forged_receipt_rejected():
    receipt = {
        "body": {
            "action": "erasure",
            "entry_id": "e1",
            "tenant_id": "t",
            "timestamp": 1,
            "dp_version": "v",
        },
        "signature_public_key": "00" * 32,
        "signature_value": "ff" * 64,
    }
    with pytest.raises(wdp.DpError) as exc:
        wdp.verify_receipt(receipt)
    assert exc.value.code == "INVALID_SIGNATURE"


def test_verify_bad_key_length():
    receipt = {"body": {"x": 1}, "signature_public_key": "00", "signature_value": "ff" * 64}
    with pytest.raises(wdp.DpError) as exc:
        wdp.verify_receipt(receipt)
    assert exc.value.code == "SIGNATURE_ENVELOPE"
