...
class JiraForwarder:
        ...
        *,
        ...
        if not webhook_url and not (base_url and api_token):
            raise IncidentForwarderError(
                "JiraForwarder requires either webhook_url or "
                "(base_url + api_token)"
            )
    ...
    def create_ticket(self, incident: dict[str, Any]) -> IncidentTicket:
        ...
        labels.append(f"aumos-incident/{incident['incident_id']}")
        ...
        url = resp.get("self") or (
            f"{self.base_url}/browse/{ticket_id}" if self.base_url else None
        )
...
class LinearForwarder:
    ...
    def __init__(
        ...
        *,
        ...
        if not api_token or not team_id:
            raise IncidentForwarderError(
                "LinearForwarder requires api_token and team_id"
            )
        self.api_token = api_token
    ...
    def create_ticket(self, incident: dict[str, Any]) -> IncidentTicket:
        ...
        itype = incident.get("incident_type")
        severity = str(incident.get("severity", "medium"))
        labels = [label_for(itype), f"aumos-incident/{incident['incident_id']}"]
        title = _build_title(incident)
        description = _build_description(incident)
    ...
    def _create_mutation(
        ...
        project_clause = (
            f", projectId: {json.dumps(self.project_id)}" if self.project_id else ""
        )
        return (
...
class MockForwarder:
    ...
    def create_ticket(self, incident: dict[str, Any]) -> IncidentTicket:
        ...
        self.calls.append(("create", {"incident_id": incident["incident_id"], "ticket_id": ticket_id}))
        return ticket
