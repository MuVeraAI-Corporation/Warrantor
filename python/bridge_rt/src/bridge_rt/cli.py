...
def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(
        prog="bridge-rt", description="Unified inference backend abstraction."
    )
    ...
    if args.cmd == "probe":
        ...
        if args.json:
            ...
            for r in rows:
                print(f"{r['backend']:<15} {r['available']!s:<10} {r['version']}")
        return 0

    if args.cmd == "generate":
        try:
            resp = bridge.generate(
                GenerateRequest(model=args.model, prompt=args.prompt, max_tokens=args.max_tokens)
            )
