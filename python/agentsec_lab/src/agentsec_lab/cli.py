...
def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(
        prog="agentsec-lab", description="Run adversarial agent scenarios against a target."
    )
    ...
    tgt.add_argument(
        "--target-refusing",
        action="store_true",
        help="Use the safe baseline target (always refuses).",
    )
    tgt.add_argument(
        "--target-compliant",
        action="store_true",
        help="Use the unsafe baseline target (always complies).",
    )
    tgt.add_argument(
        "--target-script",
        metavar="PATH",
        help="External script target (reads prompt on stdin, prints response).",
    )
    run_p.add_argument(
        "--holdouts",
        default=None,
    ...
    if args.cmd == "list":
        ...
        for scen_id, scen in lab._scenarios.items():
            if not args.include_hidden and scen.hidden:
                continue
            print(
                f"{scen_id}\t{scen.category.value}\t{scen.severity.value}\t{'hidden' if scen.hidden else 'public'}"
            )
        return 0
    ...
    if args.cmd == "run":
        if args.holdouts:
            ...
            print(
                f"agentsec-lab: loaded {n} holdout scenario(s) from {args.holdouts}",
                file=sys.stderr,
            )
        target = _make_target(args)
        result = lab.run(
            target, target_name=_target_label(args), only_public=not args.include_hidden
        )
        print(
            json.dumps(
                {
