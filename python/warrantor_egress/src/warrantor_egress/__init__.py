...
__all__ = ["EgressError", "canonical_body", "verify_receipt"]
...
def canonical_body(body: dict[str, Any]) -> str:
    """Canonical JSON of the receipt body — matches the Rust crate."""
...
def verify_receipt(receipt: dict[str, Any]) -> None:
    ...
    if not isinstance(body, dict):
        raise EgressError("MALFORMED", "receipt missing body")
    pub_hex = receipt.get("signature_public_key", "")
    sig_hex = receipt.get("signature_value", "")
    try:
        pub_bytes = bytes.fromhex(pub_hex)
    except ValueError as e:
        raise EgressError("SIGNATURE_ENVELOPE", f"public_key: {e}") from e
    if len(pub_bytes) != 32:
        raise EgressError(
    ...
    try:
        ...
        raise EgressError("SIGNATURE_ENVELOPE", f"signature: {e}") from e
    if len(sig_bytes) != 64:
        raise EgressError("SIGNATURE_ENVELOPE", f"signature must be 64 bytes, got {len(sig_bytes)}")
    ...
    try:
        ...
        if isinstance(e, InvalidSignature):
            raise EgressError("INVALID_SIGNATURE", "Ed25519 signature does not verify") from e
        raise EgressError("SIGNATURE_ENVELOPE", f"public_key: {e}") from e
