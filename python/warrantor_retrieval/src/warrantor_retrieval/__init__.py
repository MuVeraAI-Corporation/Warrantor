"""W7 Retrieval broker — Python verify path for retrieval decision receipts."""
...
__all__ = ["RetrievalError", "canonical_body", "verify_receipt"]
...
def verify_receipt(receipt: dict[str, Any]) -> None:
    ...
    if not isinstance(body, dict):
        raise RetrievalError("MALFORMED", "receipt missing body")
    pub_hex = receipt.get("signature_public_key", "")
    sig_hex = receipt.get("signature_value", "")
    try:
        pub_bytes = bytes.fromhex(pub_hex)
    except ValueError as e:
        raise RetrievalError("SIGNATURE_ENVELOPE", f"public_key: {e}") from e
    if len(pub_bytes) != 32:
    ...
    try:
        ...
        raise RetrievalError("SIGNATURE_ENVELOPE", f"signature: {e}") from e
    if len(sig_bytes) != 64:
        raise RetrievalError(
    ...
    try:
        ...
        if isinstance(e, InvalidSignature):
            raise RetrievalError("INVALID_SIGNATURE", "Ed25519 signature does not verify") from e
        raise RetrievalError("SIGNATURE_ENVELOPE", f"public_key: {e}") from e
