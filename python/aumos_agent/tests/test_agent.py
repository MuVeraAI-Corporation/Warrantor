...
from aumos_agent.cli import main as cli_main
...
class TestSurface:
    ...
    def test_public_api_exports(self) -> None:
        for name in ("AumOS", "ActionResult", "Receipt", "Finding", "ActionBlocked",
                     "ContainmentTriggered", "SecurityError", "AumOSConfig", "SideEffect"):
            assert hasattr(aumos_agent, name), f"missing public export: {name}"
...
class TestStandalonePrimitives:
    ...
    def test_run_eval(self) -> None:
        r = AumOS().run_eval("model://aumos-7b")
...
class TestConnectedMode:
    def test_issue_identity_posts_to_i1(self) -> None:
        fake = FakeBackend(responses={"issue": {"svid": "svid-real", "capability_jti": "jti-real",
                                                "verifying_key": "pk", "expires_at": 100}})
    ...
    def test_scan_secrets_uses_credential_vault(self) -> None:
        fake = FakeBackend(responses={"scan": {"findings": [{"type": "github_pat", "value": "x", "index": 0}]}})
