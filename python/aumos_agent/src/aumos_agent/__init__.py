...
class AumOSConfig:
    ...
    def resolved(self) -> dict[str, Any]:
        ...
        out = dict(_DEFAULTS)
        for f in (
            "agent_identity_url", "trust_core_bin", "flight_recorder_url", "nvtrust_bridge_url",
            "model_sbom_url", "safe_eval_url", "eval_guard_url", "kill_switch_url",
            "credential_vault_url", "defstack_bin", "http_timeout",
        ):
            v = getattr(self, f)
            if v is not None:
                out[f] = v
...
_SECRET_PATTERNS: list[tuple[str, re.Pattern[str]]] = [
    ...
    ("aws_secret_access_key", re.compile(r"\baws(?:.{0,20})?(?:secret|sk)[^\n]{0,3}[0-9a-zA-Z/+]{40}\b", re.IGNORECASE)),
...
class AumOS:
    ...
    def __init__(
        ...
        *,
        ...
        self.config = AumOSConfig(
            ...
            trust_core_bin=trust_core_bin or (_DEFAULTS["trust_core_bin"] if trust_core_url else None),
        ...
        if self.is_connected:
            try:
                out = self._run_cli(
                    ...
                    ["sign", "--data", data if isinstance(data, str) else data.decode("utf-8", "replace"),
                     "--key-id", key_id],
                )
    ...
    def verify(self, data: str | bytes, signature: str, key: str) -> bool:
        ...
        if self.is_connected:
            try:
                out = self._run_cli(
                    self._cfg("trust_core_bin"),
                    ["verify", "--data", data if isinstance(data, str) else data.decode("utf-8", "replace"),
    ...
    def _mock_sign(payload: bytes, key_id: str) -> str:
        secret = hashlib.sha256(f"aumos-mock-key:{key_id}".encode()).digest()
    ...
    def issue_identity(
        ...
        *,
        ...
        if self.is_connected:
            try:
                r = self._http_post(self._cfg("agent_identity_url"), "/v1/agent-identity:issue", body)
                r.setdefault("source", "agent-identity")
                return r
            except (RuntimeError, OSError) as e:
                return {**self._mock_issue(subject), "source": "mock", "degraded": True, "http_error": str(e)}
    ...
    def verify_identity(self, svid: str, *, audience: str = "") -> dict[str, Any]:
        ...
        if self.is_connected:
            try:
                return self._http_post(self._cfg("agent_identity_url"), "/v1/agent-identity:verify", body)
            except (RuntimeError, OSError) as e:
                valid, subject = self._mock_verify(svid)
                return {"valid": valid, "subject": subject, "source": "mock", "degraded": True, "reason": str(e)}
        valid, subject = self._mock_verify(svid)
    ...
    def revoke_identity(self, jti: str, *, reason: str = "") -> dict[str, Any]:
        ...
        if self.is_connected:
            try:
                return self._http_post(self._cfg("agent_identity_url"), "/v1/agent-identity:revoke", body)
            except (RuntimeError, OSError) as e:
    ...
    def _mock_issue(subject: str) -> dict[str, Any]:
        ...
        return {
            ...
            "verifying_key": hashlib.sha256(f"aumos-mock-key:{subject}".encode()).hexdigest()[:64],
    ...
    def emit_receipt(
        ...
        *,
        ...
        if self.is_connected:
            try:
                r = self._http_post(self._cfg("flight_recorder_url"), "/v1/flight-recorder:emit", payload)
                return Receipt(
        ...
        if self.is_connected:
            try:
                return self._http_post(
                    self._cfg("flight_recorder_url"), "/v1/flight-recorder:verify",
                    {"receipt_id": receipt_id, "signature": signature},
                )
            except (RuntimeError, OSError) as e:
                return {"valid": receipt_id.startswith("aar-"),
                        "signer": "spiffe://aumos.dev/flight-recorder",
                        "receipt_id": receipt_id, "source": "mock", "degraded": True, "reason": str(e)}
        return {"valid": receipt_id.startswith("aar-"),
                "signer": "spiffe://aumos.dev/flight-recorder",
                "receipt_id": receipt_id, "source": "mock"}
    ...
    def check_attestation(self, *, nonce: str | None = None, gpu_pci_id: str = "") -> dict[str, Any]:
        """Check a GPU attestation report via C1-1 nvtrust-bridge."""
        nonce = nonce or secrets.token_hex(16)
        body = {"nonce": nonce, "gpu_pci_id": gpu_pci_id}
        if self.is_connected:
            try:
                return self._http_post(self._cfg("nvtrust_bridge_url"), "/v1/attestation:check", body)
            except (RuntimeError, OSError) as e:
                return {**self._mock_attestation(nonce, gpu_pci_id), "source": "mock",
                        "degraded": True, "http_error": str(e)}
    ...
    def run_preflight(self, tool: str, *, inputs: str = "{}", side_effect: str = "read") -> dict[str, Any]:
        ...
        if self.is_connected:
            try:
                return self._http_post(self._cfg("eval_guard_url"), "/v1/eval-guard:preflight", body)
            except (RuntimeError, OSError) as e:
                return {**self._mock_preflight(tool, side_effect), "source": "mock",
                        "degraded": True, "http_error": str(e)}
    ...
    def _mock_preflight(tool: str, side_effect: str) -> dict[str, Any]:
        ...
        return {
            ...
            "reason": ("consequential_action_requires_approval (invariant I-08)" if consequential else "ok"),
            "tool": tool,
            "side_effect": side_effect,
            "checked_at": int(time.time()),
            "source": "mock",
    ...
    def kill(self, *, reason: str = "behavioral_anomaly", agent: str | None = None) -> dict[str, Any]:
        ...
        if self.is_connected:
            try:
                return self._http_post(self._cfg("kill_switch_url"), "/v1/kill-switch:trigger", body)
            except (RuntimeError, OSError) as e:
                return {"triggered": True, "killed_at": int(time.time()), "reason": reason,
                        "agent": ag, "source": "mock", "degraded": True, "http_error": str(e)}
        return {"triggered": True, "killed_at": int(time.time()), "reason": reason,
                "agent": ag, "source": "mock"}
    ...
    def scan_secrets(self, text: str) -> list[Finding]:
        ...
        if self.is_connected:
            try:
                r = self._http_post(self._cfg("credential_vault_url"), "/v1/credential-vault:scan", {"text": text})
                remote = r.get("findings")
                if isinstance(remote, list):
                    return [Finding(type=str(f.get("type", "unknown")),
                                    value=str(f.get("value", "****")),
                                    index=int(f.get("index", 0))) for f in remote]
    ...
    def compliance_report(self, *, scope: str = "soc2", model: str | None = None,
                          format: str = "json") -> dict[str, Any]:
        """Generate a compliance report via X1 defstack-cli (``defstack report``)."""
        if self.is_connected:
            try:
                out = self._run_cli(self._cfg("defstack_bin"),
                                    ["report", "--scope", scope, "--format", format])
                if out.returncode == 0 and out.stdout.strip():
                    return {"report_json": out.stdout.strip(), "format": format, "source": "defstack"}
            except (OSError, subprocess.SubprocessError):
                pass
        return {
            ...
            try:
                ...
                if out.returncode == 0:
                    return {"installed": True, "name": name, "version": version,
                            "source": "defstack", "stdout": out.stdout.strip()}
            except (OSError, subprocess.SubprocessError):
                pass
    ...
    def generate_sbom(self, model: str, *, format: str = "cyclonedx") -> dict[str, Any]:
        ...
        if self.is_connected:
            try:
                ...
                return {**self._mock_sbom(model), "source": "mock", "degraded": True, "http_error": str(e)}
    ...
    def _mock_sbom(model: str) -> dict[str, Any]:
        components = [
            ...
            {"type": "dataset", "name": f"{model}-instruct-tune", "bomRef": f"pkg:aumos/dataset/{model}-tune"},
        ]
        return {
            "sbom": {
                "bomFormat": "CycloneDX",
                ...
                "metadata": {"timestamp": _now_iso(), "tool": "aumos-agent-sdk"},
            },
            "format": "cyclonedx",
            "components": components,
            "source": "mock",
    ...
    def run_eval(self, model: str, pipeline_yaml: str = "") -> dict[str, Any]:
        ...
        if self.is_connected:
            try:
                ...
                return {**self._mock_eval(model), "source": "mock", "degraded": True, "http_error": str(e)}
    ...
    def _mock_eval(model: str) -> dict[str, Any]:
        return {
            ...
            "summary": {"model": model, "stages_run": ["benchmarks", "adversarial"], "passed": True},
        ...
        def decorator(fn: Callable[..., Any]) -> Callable[..., Any]:
            ...
            def wrapper(*args: Any, **kwargs: Any) -> Any:
                ...
                if self.config.fail_closed and not preflight.get("allowed", False):
                    ...
                    denied = self.emit_receipt(ag, tool, "denied", side_effect=side_effect,
                                               inputs_hash=_hash(inputs_json))
                    result = ActionResult(
                        ok=False, value=None, svid=self._svid, preflight=preflight,
                        receipt=denied, outcome="denied",
                ...
                if secret_findings:
                    preflight = {**preflight, "secret_findings": [f.as_dict() for f in secret_findings]}
                ...
                pending_receipt = self.emit_receipt(ag, tool, "pending", side_effect=side_effect,
                                                    inputs_hash=_hash(inputs_json))
                ...
                try:
                    ...
                    failure_receipt = self.emit_receipt(ag, tool, "failure", side_effect=side_effect,
                                                        inputs_hash=_hash(inputs_json))
                    result = ActionResult(
                        ok=False, value=None, svid=self._svid, preflight=preflight,
                        receipt=failure_receipt, outcome="failure",
                        duration_ms=(time.perf_counter() - start) * 1000,
                ...
                success_receipt = self.emit_receipt(ag, tool, "success", side_effect=side_effect,
                                                    inputs_hash=_hash(inputs_json))
                result = ActionResult(
                    ok=True, value=value, svid=self._svid, preflight=preflight,
                    receipt=success_receipt, outcome="success",
                    duration_ms=(time.perf_counter() - start) * 1000,
                )
                self.evidence.append(result.as_dict())
                wrapper.action_result = result  # type: ignore[attr-defined]
    ...
    def _run_cli(cmd: str, args: list[str]) -> subprocess.CompletedProcess[str]:
        ...
        return subprocess.run(
            [cmd, *args], capture_output=True, text=True, timeout=30, check=False,
        )


def _safe_serialize_args(args: tuple[Any, ...], kwargs: dict[str, Any]) -> str:
    try:
        return json.dumps({"args": list(args), "kwargs": kwargs}, default=_json_default, sort_keys=True)
