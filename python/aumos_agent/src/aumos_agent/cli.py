"""``aumos-agent`` command-line entry point.
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--version", action="version", version=f"aumos-agent {__version__}")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.cmd == "status":
        ...
        print(json.dumps({
            ...
            "endpoints": {k: cfg[k] for k in (
                "agent_identity_url", "flight_recorder_url", "kill_switch_url",
                "credential_vault_url", "eval_guard_url",
            )},
    ...
    if args.cmd == "scan-secrets":
        ...
        print(json.dumps({"count": len(findings), "findings": [f.as_dict() for f in findings]}, indent=2))
        return 0
