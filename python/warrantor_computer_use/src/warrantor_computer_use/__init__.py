"""W8 Computer-use broker — Python verify path for screen-action decision receipts."""
...
__all__ = ["ComputerUseError", "canonical_body", "verify_receipt"]
...
def verify_receipt(receipt: dict[str, Any]) -> None:
    ...
    if not isinstance(body, dict):
        raise ComputerUseError("MALFORMED", "receipt missing body")
    pub_hex = receipt.get("signature_public_key", "")
    sig_hex = receipt.get("signature_value", "")
    try:
        pub_bytes = bytes.fromhex(pub_hex)
    except ValueError as e:
        raise ComputerUseError("SIGNATURE_ENVELOPE", f"public_key: {e}") from e
    if len(pub_bytes) != 32:
    ...
    try:
        ...
        raise ComputerUseError("SIGNATURE_ENVELOPE", f"signature: {e}") from e
    if len(sig_bytes) != 64:
        raise ComputerUseError(
    ...
    try:
        ...
        if isinstance(e, InvalidSignature):
            raise ComputerUseError("INVALID_SIGNATURE", "Ed25519 signature does not verify") from e
        raise ComputerUseError("SIGNATURE_ENVELOPE", f"public_key: {e}") from e
