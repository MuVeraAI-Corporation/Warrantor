...
__all__ = ["ConformanceError", "canonical_report", "verify_report"]
...
def verify_report(signed: dict[str, Any]) -> None:
    """Verify a signed containment conformance report's Ed25519 signature."""
    report = signed.get("report")
    if not isinstance(report, dict):
        raise ConformanceError("MALFORMED", "signed report missing 'report' field")
    ...
    try:
        ...
        if isinstance(e, InvalidSignature):
            raise ConformanceError("INVALID_SIGNATURE", "Ed25519 signature does not verify") from e
        raise ConformanceError("SIGNATURE_ENVELOPE", f"public_key: {e}") from e
