"""Tests for aumos_harness: session lifecycle, file tracking, secrets, kill-switch, config parsing."""
...
def test_timeout_kills_command() -> None:
    config = HarnessConfig(working_dir=".", allowed_tools=["sleep"], max_duration_seconds=1)
    session = TrackedSession(config)
    result = session.run_agent("sleep 10", timeout=1)
    assert result["exit_code"] == -1
