...
from aumos_harness import AgentType, HarnessConfig, SideEffectClass, secure_session


def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(prog="aumos-harness", description="Run a coding agent in a secured AumOS session.")
    sub = p.add_subparsers(dest="cmd", required=True)

    run_p = sub.add_parser("run", help="Run a command in a secured session.")
    run_p.add_argument("--agent", choices=[a.value for a in AgentType], default=AgentType.GENERIC.value)
    run_p.add_argument("--dir", default=".", help="Working directory.")
    run_p.add_argument("--tools", default="git,npm,cargo,python,go,make", help="Comma-separated allowed tools.")
    run_p.add_argument("--side-effect", choices=[s.value for s in SideEffectClass], default=SideEffectClass.WRITE.value)
    run_p.add_argument("--no-kill-on-secret", action="store_true", help="Don't kill on secret exposure.")
