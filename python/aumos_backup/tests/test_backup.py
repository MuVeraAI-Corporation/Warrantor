"""Tests for aumos_backup: target validation, backup, restore, prune, edge cases."""
...
def test_prune_expired_removes_old_backups(tmp_path: Path) -> None:
    ...
    mgr = BackupManager()
    mgr.add_target(
        BackupTarget(name="f", path=str(src), backup_path=str(bk), retention_days=1)
    )
...
def test_prune_expired_zero_when_nothing_old(tmp_path: Path) -> None:
    ...
    mgr = BackupManager()
    mgr.add_target(
        BackupTarget(name="f", path=str(src), backup_path=str(bk), retention_days=30)
    )
    mgr.run_backup("f")
