"""Tests for warrantor_sla: targets, recording, windowing, status, breaches."""

from __future__ import annotations

import pytest

from warrantor_sla import SLAMonitor, SLAStatus, SLATarget
...
def test_meeting_when_all_samples_below_threshold() -> None:
    monitor = SLAMonitor()
    monitor.add_target(SLATarget("latency_ms", threshold=500, window_seconds=60, comparison="lt"))
...
def test_eq_comparison_strict() -> None:
    monitor = SLAMonitor()
    monitor.add_target(SLATarget("count", threshold=1, window_seconds=60, comparison="eq"))
...
def test_old_samples_age_out_of_window() -> None:
    monitor = SLAMonitor()
    monitor.add_target(SLATarget("m", threshold=10, window_seconds=10, comparison="lt"))
...
def test_thread_safe_concurrent_records() -> None:
    ...
    monitor = SLAMonitor()
    monitor.add_target(SLATarget("m", threshold=10_000, window_seconds=1000, comparison="lt"))
