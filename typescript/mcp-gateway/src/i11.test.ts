...
describe('I-11 — self-change is governed', () => {
  ...
  it('denies a destructive tool aimed at the policy corpus', () => {
    expect(
      isSelfChange({ toolSvid: 'spiffe://aumos.dev/policy/corpus', sideEffectClass: 'destructive' })
    ).toBe(true);
  });
  ...
  it('denies a tool that mutates a protected surface indirectly', () => {
    expect(
      isSelfChange({
        toolSvid: 'spiffe://aumos.dev/tools/generic-writer',
        sideEffectClass: 'write',
        mutates: ['spiffe://aumos.dev/trust-bundle/keys'],
      })
    ).toBe(true);
  });
  ...
  it('permits READ access to a protected surface -- inspection is not modification', () => {
    expect(
      isSelfChange({ toolSvid: 'spiffe://aumos.dev/policy/corpus', sideEffectClass: 'read' })
    ).toBe(false);
  });

  it('permits ordinary writes to unprotected resources', () => {
    expect(
      isSelfChange({ toolSvid: 'spiffe://aumos.dev/tools/github', sideEffectClass: 'write' })
    ).toBe(false);
  });
  ...
  it('is not fooled by a prefix that merely shares a leading substring', () => {
    ...
    expect(
      isSelfChange({
        toolSvid: 'spiffe://aumos.dev/policy-playground/scratch',
        sideEffectClass: 'write',
      })
    ).toBe(false);
  });
